...
We renamed this variable `dose_per_mg` to make its meaning clearer to the reader. This is a continuous variable that nevertheless tends to take on only a few values. We considered a recoding of this variable into levels, but decided against it as the dose per milligram technically may take on any value and the generalizability of our model may be improved if we are able to consider dose levels that are in a realistic range yet not present in our current data. 

```{r, fig.align='center', fig.height=2, include = FALSE}
ggplot(df, aes(x=mgstr)) + 
 geom_histogram(aes(y=..density..), colour="black", fill="white", bins = 50)+
 geom_density(alpha=.2, fill="red") +
labs(x="does per mg",y="density")
...
```{r, fig.align='center', fig.height = 2.5, warning=FALSE, message=FALSE}
plot_dose_per_mg <- ggplot(data = df[df$state=="Florida" | df$state=="North Carolina",], aes(x = dose_per_mg, y = log_ppm)) + 
  geom_point() +
...
```{r, fig.align='center', fig.height = 2.5, warning=FALSE, message=FALSE}
plot_date_num <- ggplot(data = df[df$state=="Florida" | df$state=="North Carolina",], aes(x = date_num, y = log_ppm)) + 
  geom_point() +
  geom_smooth(method=lm) +
  facet_grid(~ state)
...
box_bulk_purchase <- ggplot(data = df[df$state=="Florida" | df$state=="Texas" | df$state=="North Carolina",], aes(x = bulk_purchase, y = log_ppm, group = bulk_purchase)) + 
  geom_boxplot() +
  facet_grid(~ state) +
...
box_date_quarter <- ggplot(data = df[df$state=="Florida" | df$state=="Texas" | df$state=="North Carolina",], aes(x = date_quarter, y = log_ppm, group = date_quarter)) + 
  geom_boxplot() +
  facet_grid(~ state)

grid.arrange(box_bulk_purchase, box_date_quarter, ncol=2)
```

### Primary_Reason & source
```{r, fig.height = 2.5, fig.align='center'}
box_Primary_Reason <- ggplot(data = df[df$state=="Florida" | df$state=="North Carolina",], aes(x = Primary_Reason, y = log_ppm, group = Primary_Reason)) + 
  geom_boxplot() +
  facet_grid(~ state) +
  theme(axis.text.x = element_text(angle = 90))
```
...
```{r, fig.height = 2.5, fig.align='center'}
box_source <- ggplot(data = df[df$state=="Florida" | df$state=="North Carolina",], aes(x = source, y = log_ppm, group = source)) + 
  geom_boxplot() +
  facet_grid(~ state) + 
  theme(axis.text.x = element_text(angle = 90))

grid.arrange(box_Primary_Reason, box_source, ncol = 2)
```
