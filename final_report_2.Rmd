...
df$source <- droplevels(df$source)
knitr::kable(table(df$source), 
             col.names = c("Level", "Freq"),
             caption = "Frequency Table for Source")
```
...
#print(reason_df)
knitr::kable(reason_df_2, 
             col.names = c("Level", "Freq"),
             caption = "Frequency Table for Primary Reason")
```
...
### Date variable  
```{r}
ggplot(df, aes(x=date_num)) + 
 geom_histogram(aes(y=..density..), colour="black", fill="white", bins = 50)+
 geom_density(alpha=.2, fill="blue") 
knitr::kable(table(df$date_quarter), col.names = c("Date Quarter", "Freq"))
```
