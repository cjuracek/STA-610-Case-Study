---
title: \vspace{-1.5cm} \textbf{STA610 Case Study 1 Team 4 Report}
author:
...
knitr::opts_chunk$set(echo = FALSE, cache=TRUE, fig.height = 3, tidy.opts=list(width.cutoff=60), tidy=TRUE)
```
...
mod_s <- lmer(data=df, log_ppm ~ (1 |state) + date_num + date_quarter + 
                  dose_per_mg + bulk_purchase + Primary_Reason + source, REML=F)
mod_c <- lmer(data=df, log_ppm ~ (1 |city) + date_num + date_quarter + 
                  dose_per_mg + bulk_purchase + Primary_Reason + source, REML=F)
mod_r <- lmer(data=df, log_ppm ~ (1 |USA_region) + date_num + date_quarter +
                  dose_per_mg + bulk_purchase + Primary_Reason + source, REML=F)
```
...
drop_date_num <- anova(mod_s,
      lmer(data=df, log_ppm ~ (1 |state) + date_quarter + dose_per_mg + 
               bulk_purchase + Primary_Reason + source, REML=F))
date_num_pval <- drop_date_num$`Pr(>Chisq)`[2]
# Test date_quarter
drop_quarter <- anova(lmer(data=df, log_ppm ~ (1 |state) + 
                        date_quarter + dose_per_mg +
                        bulk_purchase + Primary_Reason + source, REML=F),
                lmer(data=df, log_ppm ~ (1 |state) + dose_per_mg + 
                         bulk_purchase + Primary_Reason + source, REML=F))
quarter_pval <- drop_quarter$`Pr(>Chisq)`[2]

# Test dose_per_mg
drop_dose <- anova(lmer(data=df, log_ppm ~ (1 |state) + dose_per_mg + 
                        bulk_purchase + Primary_Reason + source, REML=F),
                   lmer(data=df, log_ppm ~ (1 |state) + bulk_purchase 
                        + Primary_Reason + source, REML=F))
dose_pval <- drop_dose$`Pr(>Chisq)`[2]

# Test bulk purchase
drop_bulk <- anova(lmer(data=df, log_ppm ~ (1 |state) + dose_per_mg + 
                        bulk_purchase + Primary_Reason + source, REML=F),
      lmer(data=df, log_ppm ~ (1 |state) + dose_per_mg + 
               Primary_Reason + source, REML=F))
...
# Test primary reason
drop_reason <- anova(lmer(data=df, log_ppm ~ (1 |state) + dose_per_mg + 
                              bulk_purchase + Primary_Reason + source, REML=F),
      lmer(data=df, log_ppm ~ (1 |state) + dose_per_mg + 
...
# Test source
drop_source <- anova(lmer(data=df, log_ppm ~ (1 |state) + dose_per_mg + 
                              bulk_purchase + Primary_Reason + source, REML=F),
      lmer(data=df, log_ppm ~ (1 |state) + dose_per_mg + 
...
*source* (“Source: Unknown” is the reference, other levels ommitted for brevity): Controlling for other variables, purchasing oxymorphone with Internet source tends to increase the price per mg by a multiplicative effect of exp(0.1339468) = 1.143332, about a 14.33% increase than the price per mg of oxymorphone for people with unknown drug source.
