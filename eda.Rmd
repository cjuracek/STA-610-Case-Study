...
levels(streetrx$source)[levels(streetrx$source) == ''] <- NA_character_
streetrx$source <- recode_factor(streetrx$source, 'w'=NA_character_,
                                 'Idk'=NA_character_)

levels(streetrx$source)[grepl('bluelight', levels(streetrx$source), fixed=TRUE)] <- 'bluelight'
levels(streetrx$source)[grepl('opiophile', levels(streetrx$source), fixed=TRUE)] <- 'opiophile'
```
...
```{r}
ggplot(small_ppm, aes(x=bulk_purchase, y=ppm)) + geom_boxplot()
```
...
```{r}
t.test(ppm ~ bulk_purchase, data = small_ppm)
```

So bulk purchase looks to be a significant predictor *if* we exclude the extreme ppm values
