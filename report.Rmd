...
resi <- plot_redres(mod1, type = "std_cond")
grid.arrange(resi, resqq, ranefci, ncol = 3)
```

Model diagnostics discussions here.

From the esiduals versus the fitted values plot, there are several outliers in the lower half of the plot. 
From the normal quantile plot of the raw conditional residuals, the residuals are skewed and there are a lot of outliers. Thus, the residuals are not normally distributed. 
...
* date_num: For the date_num variable, controlling for other variables, for every one unit increase in the date_num, the price per mg of oxymorphone reduces by a multiplicative effect of exp(-0.0001290) =  0.999871, about a 0.0129% decrease.

* mgstr: For the mgstr variable, controlling for for other variables, for every one unit increase in mgstr, the price per mg of oxymorphone reduces by a multiplicative effect of exp(-0.0241262) =  0.9761625, about a 2.38375% decrease.

* bulk_purchase: “Not bulk purchase” is the reference group.
  * Controlling for other variables, being bulk purchase reduces the price per mg by a multiplicative effect of exp(-0.161) = 0.8521, about a 14.79% reduction.

* Primary_Reason: “Primary_Reason: Other or Unknown” is the reference group. 
  * Controlling for other variables, taking oxymorphone with the primary reason of self-treating increase the price per mg by a multiplicative effect of exp(0.05) = 1.0513, about a 5.13% increase than the price per mg of oxymorphone for people whose primary reason is “other or unknown”. 
  * Controlling for other variables, taking oxymorphone with the primary reason of enjoyment tends to increase the price per mg by a multiplicative effect of exp(0.2) = 1.221403, about a 22.1403% increase than the price per mg of oxymorphone for people whose primary reason is “other or unknown”.

* source: “Source: Unknown” is the reference group. 
  * Controlling for other variables, purchasing oxymorphone with Internet source tends to increase the price per mg by a multiplicative effect of exp(0.02) = 1.0202, about a 2.02% increase than the price per mg of oxymorphone for people with unknown drug source.
  * Controlling for other variables, purchasing oxymorphone with personal source tends to increase the price per mg by a multiplicative effect of exp(0.07) = 1.0725, about a 7.25% increase than the price per mg of oxymorphone for people with unknown drug source.
  * Controlling for other variables, purchasing oxymorphone with “heard it” source tends to increase the price per mg by a multiplicative effect of exp(0.24) = 1.2712, about a 27.12% increase than the price per mg of oxymorphone for people with unknown drug source.
...
```
Intraclass correlation is 0.047, which means values are not very similar from the same state.
